...
function [Disc,R,t,MM,CM,A] = xDiscrepancyModified(File1,Model,File2,Cand,LocationWeight,AngleWeight)
...
elseif (length(LocationWeight) == L), % more than two nucleotides, use bases
  ...
  t = CMean'-inv(R)*ModelWeightedCenter';

  S = LocationWeight * sum(((MCC' - R*CC').^2))';  % distances between centers
  S2 = LocationWeight * sum(((ModelCenters' - R * (CandiCenters'-t*ones(1,L))).^2))';
