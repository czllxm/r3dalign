...
%     clearvars -except Band Neigh Molecule Chain NTList1 NTList2 Query d m1 m2;
      clearex ('Band','Neigh','Disc','Molecule','Chain','NTList1','NTList2','Query','d','m1','m2');
      [Molecule{m1} ' ' Molecule{m2}]
      for iter=1:3
        for i=1:length(Band{iter}(:,1))
          for j=1:length(Neigh{iter}(:,1))
            for j1=1:length(Disc{iter}(:,1))
               clear B P D;
               for k=1:iter
                 B{k}=Band{iter}(i,k);
                 P{k}=Neigh{iter}(j,k);
...
% %     m1 m2;
%     clearex('Band','Neigh','Disc','Molecule','Chain2','Chain','NTList1','NTList2','Query','d','m1','m');
